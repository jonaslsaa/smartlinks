...
function factsHtml(decoded: DecodedPayload): string {
  ...
  const secrets = facts.sealedSecrets.length
    ...
    : "None";
  return `<section class="panel" aria-labelledby="facts-heading"><h2 class="eyebrow" id="facts-heading">Smartlink facts</h2><dl><dt>Payload version</dt><dd>${facts.payloadVersion}</dd><dt>Confirmation required</dt><dd>${facts.interstitial ? "Yes" : "No"}</dd><dt>Crawler execution</dt><dd>${facts.allowCrawlers ? "Allowed" : "Blocked"}</dd><dt>Expiry</dt><dd>${escapeHtml(expiry)}</dd><dt>Sealed secrets</dt><dd>${secrets}</dd><dt>Compile closures</dt><dd>${facts.compileClosures}</dd></dl></section>`;
}
